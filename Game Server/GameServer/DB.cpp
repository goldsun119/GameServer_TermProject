...
void set_DB_Info( int ci ) {
	...
	if ( retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO ) {
		...
		sprintf( buf, "EXEC dbo.Set_id %s, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d", g_clients[ci].game_id, g_clients[ci].x, g_clients[ci].y, g_clients[ci].level, g_clients[ci].hp, g_clients[ci].Max_hp, g_clients[ci].exp, g_clients[ci].skill_1, g_clients[ci].skill_2, g_clients[ci].skill_3 );
		MultiByteToWideChar( CP_UTF8, 0, buf, strlen( buf ), sql_data, sizeof sql_data / sizeof *sql_data );
		sql_data[strlen( buf )] = '\0';
