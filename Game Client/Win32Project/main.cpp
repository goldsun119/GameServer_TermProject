...
LRESULT CALLBACK WndProc( HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM  lParam ) {
	...
	case WM_KEYDOWN:
	{
		if ( wParam == '1' || wParam == '2' || wParam == '3' || wParam == '4' || wParam == '5' ) {
			...
			// 4 = (81, 184)
			// 5 = (253, 282)
			if ( wParam == '1' )Send_Move_Packet( 1 );
			if ( wParam == '2' )Send_Move_Packet( 2 );
			if ( wParam == '3' )Send_Move_Packet( 3 );
			if ( wParam == '4' )Send_Move_Packet( 4 );
			if ( wParam == '5' )Send_Move_Packet( 5 );
			break;
		}
...
void init() {
	for ( int i = 0; i < WORLD_HEIGHT; ++i ) { // 가로
		for ( int j = 0; j < WORLD_WIDTH; ++j ) { // 세로
			if ( i % 2 == j % 2 )
				isGameData[i][j] = 1;
		}
	}
...
void ProcessPacket( char *ptr )
{
	...
	case SC_STAT_CHANGE: {
		...
		if ( id == g_myid ) {
			...
			npc[id - NPC_ID_START].skillTimer_1 = my_packet->skill_1;
			npc[id - NPC_ID_START].skillTimer_2 = my_packet->skill_2;
			npc[id - NPC_ID_START].skillTimer_3 = my_packet->skill_3;
		}
...
void SendPacket( void *packet) {
	...
	char 	send_buffer[BUF_SIZE];
		...
		int ptype = reinterpret_cast< char *>(packet)[1];
		memcpy(send_buffer, packet, psize);
		send_wsabuf.buf = reinterpret_cast<CHAR *>(send_buffer);
		send_wsabuf.len = psize;
		int res = WSASend(g_mysocket, &send_wsabuf, 1, NULL, 0, NULL, NULL);
}
